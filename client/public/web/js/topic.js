$(function () {
    ...
    $('#share-btn').click(function(){
       $('#topic-share').toggle(300);
    });
    
    $('#playlist-list, #list-topic-song').slimScroll({
        height: 'auto',
        railVisible: true,
        railColor: '#fff',
        color: '#ccc'
...
function showAddPlaylist(atid) {
    ...
    $.get("./ajaxprocess.se", db, function (re) {
        if (re.status ==1) {
            $("#playlist-list").empty();
            var str_temp = "";
...
function createRowPlaylist(atid, elem) {
    ...
    str_temp +=  '<div class="playlist-list-item">'
        ...
        + '<a href="javascript:void(0);" onclick="addToPlaylist('+atid+','+elem._id+')" title="'+elem.name+'"'
        + 'id="playlist-'+elem._id+'"><i class="fa fa-music"></i> &nbsp; ' + elem.name+'</a>'
        + '</div>'
        +'</div>';
    return str_temp;
}
...
function addComment(atid,pid,txtvalue,objhtml,type) {
    ...
    $('#loading').show();
    $.post('./sercurity.se',db,function(data){
        if(data.status==1) {
...
function loadComment(atid,pid,objhtml, type) {
    ...
    $.post('./ajaxprocess.se',db,function(data){
        if(data.status==1 )
        {
            if (data.data) {
...
$(document).ready(function () {
    (function(d, s, id) {
      ...
      js.src = "//connect.facebook.net/vi_VN/all.js#xfbml=1&appId=768835433173463";
