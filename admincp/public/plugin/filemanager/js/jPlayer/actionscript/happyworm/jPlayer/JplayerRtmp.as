...
public class JplayerRtmp extends Sprite {
        ...
        switch (e.info.code) {
            ...
            case "NetStream.Play.Start" :
                ...
                if (firstTime) {
                    ...
                    // Bascally, we assume that after a setMedia, you will music it.
            ...
            case "NetStream.Play.Stop" :
                ...
                if (Math.abs((getDuration() - getCurrentTime())) < 150) {// Testing found 150ms worked best for M4A files, where playHead(99.9) caused a stuck state due to firing with ~116ms left to music.
                    //endedEvent();
                }
                break;
        ...
        if (!myConnection.connected) {
            ...
            if (!myStatus.metaDataReady) {
                ...
                if (myStatus.playOnLoad) {
                    ...
                    if (myStatus.pausePosition > 0) {// Important for setMedia followed by music(time).
                        play(myStatus.pausePosition);
                    }
                    else {
                        play();// Not always sending pausePosition avoids the extra seek(0) for a normal music() command.
                    }

                }
                else {
                    pause(myStatus.pausePosition);// Always send the pausePosition. Important for setMedia() followed by pause(time). Deals with not reading stream.time with setMedia() and music() immediately followed by stop() or pause(0)
                }
    ...
    public function play(time:Number = NaN):Boolean {
        ...
        if (myStatus.isStartingDownload) {
            ...
            if (myStatus.metaDataReady && myStatus.pausePosition > myStatus.duration && connectString.streamTYPE != "LIVE") { // The time is invalid, ie., past the end.
                ...
                timeUpdates(false);
                timeUpdateEvent();
                if (wasPlaying) { // For when playing and then get a music(huge)
    ...
    public function pause(time:Number = NaN):Boolean {
        //trace("PAUSE: "+time);
        myStatus.playOnLoad = false;// Reset flag in case load/music issued immediately before this command, ie., before onMetadata() event.
        myStatus.playOnSeek = false;// Reset flag in case music(time) issued before the command and is still seeking to time set.
