...
public class JplayerMp3 extends Sprite {
    ...
    private function loadOpen(e:Event):void {
        ...
        if (myStatus.playOnLoad) {
            ...
            // this.dispatchEvent(new JplayerEvent(JplayerEvent.JPLAYER_LOADSTART, myStatus)); // So loadstart event happens before music event occurs.
            play();
        } else {
    ...
    public function pause(time:Number = NaN):Boolean {
        myStatus.playOnLoad = false; // Reset flag in case load/music issued immediately before this command, ie., before loadOpen() event.
        ...
        if (myStatus.isPlaying) {
            myStatus.isPlaying = false;
            myChannel.stop();
            if (myChannel.position > 0) { // Required otherwise a fast music then pause causes myChannel.position to equal zero and not the correct value. ie., When it happens leave pausePosition alone.
