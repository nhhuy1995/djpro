                                        ...
                                        <div class="sortable" id="listOfNewInSlide">
                                            {% for newsDetail in slideshow %}
                                                <div class="lv-item media">
                                                    ...
                                                    <div class="pull-left">
                                                        <img alt="" src="{{ newsDetail['priavatar'] }}" class="lv-img-sm">
                                                    </div>
                                                    <div class="media-body">
                                                        <div class="lv-title">{{ newsDetail['name'] }}</div>

                                                        <ul class="lv-attrs">
                                                            <li>Date Created: {{ date('d-m-Y',newsDetail['datecreate']) }}</li>
                                                        </ul>
                                                    </div>
                                                </div>
                                            {%  endfor %}
...
<script type="text/javascript">
    ...
    function getcategory(obj) {
        ...
        thread = setTimeout(function () {
            ...
            $('#sugges_data').html('');
            if (q.length) {
                $.get("/incoming/getlistsong", {q: q}, function (re) {
                    var data = re.data;
                    if (data.length > 0) {
                        data.forEach(function (entry) {
                            var htmlx = '<li onclick="returncategory(this)" +'
                                    + ' data-name="' + escapeString(entry.name) + '"'
                                    + ' data-id="' + entry._id + '"'
