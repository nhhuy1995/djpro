...
class UsersController extends ControllerBase
{
    ...
    public function indexAction()
    {
        ...
        foreach($list_user as $key=>$item){
            $list_user[$key]['link'] = DOMAIN.Makelink::link_view_member($item['username'],$item['_id']);
        }
        $count = Users::count();
        $this->view->listUser = $list_user;
        $this->view->listrole = Role::findAndReturnArray(array());
    ...
    public function formprocessAction()
    {
        if ($this->request->isPost()) {
            ...
            if(empty($postvalue['password'])) unset($postvalue['password']);
